...
export const Step4Company: React.FC = () => {
  const navigate = useNavigate();
  const { currentCompany, setCurrentCompany, setCurrentStep } = useOnboardingStore();
  const [company, setCompany] = useState(currentCompany);
  const [isValid, setIsValid] = useState(!!currentCompany);

  useEffect(() => {
    setIsValid(company.trim().length > 0);
  }, [company]);
      ...
      <div className="w-full max-w-xl">
        <div className="flex items-start mb-16">
          ...
          <div className="ml-4 bg-gray-100 p-4 rounded-xl rounded-tl-none">
            <p className="text-lg font-medium">What's your current company name?</p>
          </div>
        </div>
