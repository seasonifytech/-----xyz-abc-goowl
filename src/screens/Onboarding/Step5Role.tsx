...
export const Step5Role: React.FC = () => {
  const navigate = useNavigate();
  const { currentRole, setCurrentRole, setCurrentStep } = useOnboardingStore();
  const [role, setRole] = useState(currentRole);
  const [isValid, setIsValid] = useState(!!currentRole);

  useEffect(() => {
    setIsValid(role.trim().length > 0);
  }, [role]);
      ...
      <div className="w-full max-w-xl">
        <div className="flex items-start mb-16">
          ...
          <div className="ml-4 bg-gray-100 p-4 rounded-xl rounded-tl-none">
            <p className="text-lg font-medium">What is your current job title or role?</p>
          </div>
        </div>
